...
/**
 * @file       ble_gap.c
 * @note       ble gap interface source file
 * @attention
 */
/*############################################################################*/
...
int BLE_GapReplyPairingFeature(BLE_GapConnHandle connHandle)
{
	...
	//ecKeyset.keys_own.p_enc_key  = &gapMem.wrapperBondInfo.ownEncKey;
	//secKeyset.keys_own.p_id_key   = &gapMem.wrapperBondInfo.ownIdKey;
...
int BLE_GapSaveBondInfo(BLE_GapBondInfo *info)
{
	...
	BLE_PRT("BLE_GapSaveBondInfo: bleBondEnableList 0x%x\n", bleBondEnableList);
...
int BLE_GapClearBondInfo(BLE_GapBondInfo *info)
{
	...
	BLE_PRT("BLE_GapClearBondInfo: new bleBondEnableList 0x%x\n", bleBondEnableList);
	if (index == BLE_SAVE_BOND_DEVICE_MAX_NUM) {
		ret = -ENOENT;
