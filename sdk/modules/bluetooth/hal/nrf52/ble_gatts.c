...
/**
 * @file       ble_gatts.c
 * @note       ble gatt server interface source file
 * @attention
 */
/*############################################################################*/
 ...
 * Function
 *****************************************************************************/
static int setPermission(ble_gap_conn_sec_mode_t *perm, BLE_SecMode secMode)
{
	int ret = 0;
	switch( secMode ) {
...
int BLE_GattsAddCharacteristic(uint16_t serviceHandle, BLE_CharMeta const* charMeta, BLE_GattsAttr const* attrCharValue, BLE_GattsCharHandles* handles)
{
	...
	BLE_SecMode         noAccessMode = BLE_SEC_MODE_NO_ACCESS;
